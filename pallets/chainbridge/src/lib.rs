...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		#[allow(non_snake_case)]
		fn ProposalLifetime() -> T::BlockNumber {
			T::ProposalLifetime::get()
		}

		#[allow(non_snake_case)]
		fn BridgeAccountId() -> T::AccountId {
	...
	pub type Resources<T: Config> =
		StorageMap<_, Blake2_256, ResourceId, Option<Vec<u8>>, ValueQuery>;
	...
	impl<T: Config> Pallet<T> {
		...
		fn bump_nonce(id: ChainId) -> DepositNonce {
			let nonce = Self::chains(id).unwrap() + 1;
